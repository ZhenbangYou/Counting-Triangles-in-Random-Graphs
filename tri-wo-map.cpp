...
static constexpr int kRandMax = std::numeric_limits<int>::max();
...
static constexpr int kSigmas = 4;
...
static constexpr int kRepeat = static_cast<int>(1e5);

int main() {
  auto begin_timepoint = std::chrono::high_resolution_clock::now();

  constexpr IndexType n_vals[]{1000, 100};
  constexpr IndexType d_vals[]{2, 3, 6};
  constexpr int num_of_n = sizeof(n_vals) / sizeof(IndexType);
  constexpr int num_of_d = sizeof(d_vals) / sizeof(IndexType);
